...
\aufgabe{5}
  ...
  \begin{tikzpicture}[scale=1,every edge/.style={draw=blue, very thick}]
    ...
    \node[relationship] (r1) [below=of halle] {enthält} edge node[right]{(1,1)} (halle);

    \node[entity] (stand) [below=of r1] {Stand} edge[double, line width=1pt] node[right]{(1,*)} (r1);
      \node[attribute] [below=of stand] {Nummer} edge (stand);

    \node[relationship] (r2) [right=of stand] {mietet} edge[double, line width=1pt] node[above]{(1,*)} (stand);

    \node[entity] (firma) [below=of r2] {Firma} edge node[right]{(1,1)} (r2);
      \node[attribute] [right=of firma] {\underline{Name}} edge (firma);

    \node[ident relationship] (r3) [below=of firma] {belegt um} edge node[right]{(0,*)} (firma);
